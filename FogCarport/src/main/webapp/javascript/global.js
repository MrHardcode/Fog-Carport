...
function prepareShedMenu() {
    ...
    function fillDropDownShedDimensions() {
        ...
        for (i in shedLength) {
            lengthOptions.options[lengthOptions.options.length] = new Option(shedLength[i], shedLength[i]);
        }
        //This enhanced loop creates a new option in the select for the shed-dimensions
        for (j in shedWidth) {
            widthOptions.options[widthOptions.options.length] = new Option(shedWidth[j], shedWidth[j]);
        }
    }
